...
Antes de cerrar, **abrir la página de [diagramas de arquitectura](diagramas-arquitectura.html)** y recorrerla en vivo. Son 5 visualizaciones que vamos a usar como referencia toda la diplomatura, en orden de zoom-out → zoom-in → tactical:
...
2. **Las 3 capas — mapa global** — el modelo mental de más alto nivel: edge (browser + wallet) · web2 (backend, indexer, Postgres, IPFS, observabilidad) · on-chain (contrato + roles + state). Regla práctica: si necesitan firma + gas → es on-chain; si no, es web2.
3. **Arquitectura real del sistema de credenciales** — el zoom-in del anterior: Frontend (Vercel) + Backend Web2 + `AcademicCredentials.sol` en Sepolia + IPFS para PDFs/fotos + indexer + observabilidad. **Es la referencia que van a usar para el TP final.**
4. **Read vs Write — `cast call` vs `cast send`** — la confusión #1 de todo principiante. Misma sintaxis, comportamiento radicalmente distinto.
5. **Qué vive dónde — público vs privado** — qué pueden compartir y qué nunca: lo rojo (private key, frase BIP-39, `.env`) es secreto, lo verde (address, txs, bytecode, storage) es público por diseño.
